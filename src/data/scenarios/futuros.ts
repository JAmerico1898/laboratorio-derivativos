...
export const FUTUROS_SCENARIOS: Scenario[] = [
  {
    ...
    context: {
      narrative: "Você é gestor(a) de um fundo de renda fixa com **R$ 500 milhões** em títulos prefixados (NTN-F e LTN) carregados a um **yield médio de 12,50% a.a.** (a mercado, igual ao DI Jul/27) e com duration média de **3 anos** (≈ **756 d.u.**). O CDI está em **11,75% a.a.** e o DI futuro de Jul/27 está em **12,50% a.a.**. São **630 d.u.** até o vencimento do DI. O Copom se reúne em 2 semanas e o mercado está dividido entre manutenção e alta de 50bps. Se os juros subirem, seus títulos perdem valor.",
      marketData: { spotRate: 11.75, forwardRate90d: 12.50, cdiRate: 0.1175, notional_usd: 599966622, tenor: 630, pnlMethod: "di_pu_mtm", nContracts: 8054, duDays: 630, portfolioValue: 500000000, portfolioDuration: 3, portfolioRate: 12.50, portfolioDu: 756 },
      displayFields: [["CDI atual", "11,75% a.a."], ["DI Jul/27", "12,50% a.a."], ["Yield carteira", "12,50% a.a."], ["PL Fundo", "R$ 500M"], ["Duration", "3 anos (756 d.u.)"], ["PU Jul/27", "74.493"]],
    ...
    steps: [
      ...
      { id: "pu_calc", type: "choice", prompt: "Antes de calcular o número de contratos, você precisa determinar o PU (Preço Unitário) do DI Jul/27. A taxa é 12,50% a.a. e faltam 630 dias úteis até o vencimento. Recorde: PU = 100.000 ÷ (1 + taxa)^(DU/252). Qual é o PU?", choices: [
      ...
      { id: "contract_di", type: "choice", prompt: "Agora calcule o número de contratos. O PU é ~74.493 (nocional real por contrato). A carteira tem R$ 500M e duration de 3 anos. O DI Jul/27 tem duration de ~2,5 anos. Quantos contratos de DI futuro devemos comprar?", choices: [
        { id: "di_nocional", contracts: 6712, label: "6.712 contratos — matching nocional pelo PU (R$ 500M ÷ R$ 74.493)", correct: false, score: 10, feedback: "Você acertou em usar o PU real ao invés do valor de face — isso é importante! Porém, matching nocional ignora a diferença de duration. Com duration da carteira (3a) > duration do futuro (2,5a), é preciso ajustar por esse fator.", next: "resolution_di" },
        { id: "di_duration", contracts: 8054, label: "8.054 contratos — nocional pelo PU ajustado por duration: (500M ÷ 74.493) × (3,0 ÷ 2,5)", correct: true, score: 20, feedback: "Excelente! O cálculo correto em dois passos: (1) Contratos base = R$ 500M ÷ PU 74.493 ≈ 6.712 contratos. (2) Ajuste por duration = 6.712 × (3,0 / 2,5) = 6.712 × 1,2 ≈ 8.054 contratos. Usar o PU real (não o valor de face) e ajustar por duration garante que a sensibilidade do hedge case com a da carteira.", next: "resolution_di" },
        { id: "di_face", contracts: 6000, label: "6.000 contratos — usando valor de face (500M ÷ 100k) × (3,0 ÷ 2,5)", correct: false, score: 5, feedback: "Você acertou no ajuste por duration (× 1,2), mas usou o valor de face (R$ 100k) ao invés do PU real (R$ 74.493). O nocional efetivo de cada contrato hoje é o PU, não o valor de face. Usando o PU: 500M ÷ 74.493 × 1,2 ≈ 8.054 contratos. A diferença é significativa — 6.000 vs 8.054 — e resulta em sub-hedge.", next: "resolution_di" },
      ]},
      { id: "resolution_di", type: "resolution", prompt: "O Copom decidiu e o mercado reagiu.", scenarios: [
        { id: "juros_subiram", label: "Cenário A: DI Jul/27 subiu para 13,50%", fixingRate: 13.50, description: "Copom surpreendeu com alta de 75bps na Selic." },
        { id: "juros_cairam", label: "Cenário B: DI Jul/27 caiu para 11,50%", fixingRate: 11.50, description: "Surpresa dovish: Copom cortou a Selic e indicou ciclo de queda." },
        { id: "juros_estaveis", label: "Cenário C: DI Jul/27 estável em 12,60%", fixingRate: 12.60, description: "Copom manteve a taxa como esperado." },
      ]},
    ],
  },
  {
