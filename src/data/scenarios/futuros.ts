...
export const FUTUROS_SCENARIOS: Scenario[] = [
  {
    ...
    context: {
      narrative: "Você é gestor(a) de um fundo de renda fixa com **R$ 500 milhões** em títulos prefixados (NTN-F e LTN) carregados a um **yield médio de 12,80% a.a.** (a mercado, igual ao DI Jan/28) e com duration média de **3 anos** (≈ **756 d.u.**). O CDI está em **11,75% a.a.** e o DI futuro de Jan/28 está em **12,80% a.a.**, com **756 d.u.** até o vencimento — exatamente a duration da carteira. O Copom se reúne em 2 semanas e o mercado está dividido entre manutenção e alta de 50bps. Se os juros subirem, seus títulos perdem valor.",
      marketData: { spotRate: 11.75, forwardRate90d: 12.80, cdiRate: 0.1175, notional_usd: 499982877, tenor: 756, pnlMethod: "di_pu", nContracts: 7176, duDays: 756, portfolioValue: 500000000, portfolioDuration: 3, portfolioRate: 12.80, portfolioDu: 756 },
      displayFields: [["CDI atual", "11,75% a.a."], ["DI Jan/28", "12,80% a.a."], ["Yield carteira", "12,80% a.a."], ["PL Fundo", "R$ 500M"], ["Duration", "3 anos (756 d.u.)"], ["PU Jan/28", "69.674"]],
    ...
    steps: [
      ...
      { id: "pu_calc", type: "choice", prompt: "Antes de calcular o número de contratos, você precisa determinar o PU (Preço Unitário) do DI Jan/28. A taxa é 12,80% a.a. e faltam 756 dias úteis até o vencimento. Recorde: PU = 100.000 ÷ (1 + taxa)^(DU/252). Qual é o PU?", choices: [
        { id: "pu_correct", label: "PU ≈ 69.674 — calculado como 100.000 ÷ (1,1280)^(756/252)", correct: true, score: 20, feedback: "Excelente! PU = 100.000 ÷ (1,128)^3 = 100.000 ÷ 1,435249 ≈ 69.674. Esse é o valor presente de R$ 100.000 descontado pela taxa do DI a 756 dias úteis. O nocional real de cada contrato hoje é R$ 69.674 — não R$ 100.000! O valor de face (R$ 100k) só é atingido no vencimento.", next: "contract_di" },
        { id: "pu_face", label: "PU = 100.000 — o nocional do contrato", correct: false, score: 0, feedback: "R$ 100.000 é o valor de face no vencimento, não o PU de hoje. O PU é o valor presente, obtido descontando R$ 100.000 pela taxa de juros e prazo: PU = 100.000 ÷ (1,128)^(756/252) = 100.000 ÷ 1,435249 ≈ 69.674. Essa distinção é fundamental — o nocional real de cada contrato hoje é R$ 69.674.", next: "contract_di" },
        { id: "pu_wrong_formula", label: "PU ≈ 87.200 — calculado como 100.000 × (1 − 0,128)", correct: false, score: 5, feedback: "Cuidado! O desconto no DI futuro é composto, não simples. A fórmula correta é PU = 100.000 ÷ (1 + taxa)^(DU/252), usando capitalização composta com base em 252 dias úteis. Com taxa de 12,80% e 756 DU: PU = 100.000 ÷ (1,128)^3 ≈ 69.674. Desconto linear geraria um valor incorreto.", next: "contract_di" },
      ]},
      { id: "contract_di", type: "choice", prompt: "Agora calcule o número de contratos. O PU é ~69.674 (nocional real por contrato) e a carteira tem R$ 500M. Atenção: a duration da carteira (3 anos) e a do DI Jan/28 (756 d.u. = 3 anos) são iguais. Quantos contratos de DI futuro devemos comprar?", choices: [
        { id: "di_nocional", contracts: 7176, label: "7.176 contratos — casamento nocional pelo PU (R$ 500M ÷ R$ 69.674)", correct: true, score: 20, feedback: "Excelente! Como a duration da carteira (3 anos) é igual à do DI Jan/28 (756 d.u. = 3 anos), NÃO há descasamento a corrigir — o casamento nocional pelo PU já iguala as sensibilidades. Cálculo: R$ 500M ÷ R$ 69.674 ≈ 7.176 contratos. O ponto crítico é usar o PU real (R$ 69.674), não o valor de face (R$ 100.000). Se as durations fossem diferentes, aí sim seria preciso multiplicar pela razão entre elas.", next: "resolution_di" },
        { id: "di_face", contracts: 5000, label: "5.000 contratos — usando o valor de face (R$ 500M ÷ R$ 100.000)", correct: false, score: 5, feedback: "Você usou o valor de face (R$ 100k) ao invés do PU real (R$ 69.674). O nocional efetivo de cada contrato hoje é o PU, não o valor de face — o contrato só vale R$ 100.000 no vencimento. Usando o PU: 500M ÷ 69.674 ≈ 7.176 contratos. A diferença é grande — 5.000 vs 7.176 — e resulta em sub-hedge de ~30% da carteira.", next: "resolution_di" },
        { id: "di_duration", contracts: 8611, label: "8.611 contratos — nocional pelo PU ajustado por duration: (500M ÷ 69.674) × (3,0 ÷ 2,5)", correct: false, score: 10, feedback: "Você acertou em usar o PU real — isso é importante. Mas aplicou um ajuste de duration que não existe aqui: o DI Jan/28 tem 756 d.u. = 3,0 anos, exatamente a duration da carteira. A razão correta é 3,0 ÷ 3,0 = 1,0, não 3,0 ÷ 2,5 = 1,2. Multiplicar por 1,2 gera 8.611 contratos e sobre-hedge de 20% — o excedente vira exposição direcional, não proteção. Antes de aplicar a receita, verifique se há descasamento a corrigir.", next: "resolution_di" },
      ]},
      { id: "resolution_di", type: "resolution", prompt: "O Copom decidiu e o mercado reagiu.", scenarios: [
        { id: "juros_subiram", label: "Cenário A: DI Jan/28 subiu para 13,50%", fixingRate: 13.50, description: "Copom surpreendeu com alta da Selic e a curva abriu 70bps no vértice de 3 anos." },
        { id: "juros_cairam", label: "Cenário B: DI Jan/28 caiu para 11,50%", fixingRate: 11.50, description: "Surpresa dovish: Copom cortou a Selic e indicou ciclo de queda, derrubando a curva em 130bps." },
        { id: "juros_estaveis", label: "Cenário C: DI Jan/28 estável em 12,60%", fixingRate: 12.60, description: "Copom manteve a taxa como esperado; a curva cedeu apenas 20bps." },
      ]},
    ],
  },
  {
  ...
  },
  {
    ...
    context: {
      narrative: "Você é estrategista sênior de renda fixa no **Asset Management Carioca**. A curva DI mostra **DI Jan/27 a 12,00% a.a. (504 d.u.)** e **DI Jan/28 a 12,80% a.a. (756 d.u.)**, formando um spread Jan/28 − Jan/27 de **80bps**. Os PUs iniciais são: **PU₀_curto = 100.000 ÷ (1+12%)^(504/252) = R$ 79.719,39** e **PU₀_longo = 100.000 ÷ (1+12,80%)^(756/252) = R$ 69.674,31**. A sensibilidade do PU a uma variação de 1bp é, pela derivada da fórmula PU (∂PU/∂r ≈ −PU × (du/252) / (1+r)): **R$ 14,24/contrato no curto** e **R$ 18,53/contrato no longo**. Historicamente, quando o Copom inicia um ciclo de corte, o vértice longo cai mais em magnitude que o curto e o spread fecha para **30-40bps**. Exemplo: se Jan/27 cair para 10,00% e Jan/28 cair para 10,30%, o spread sai de 80bps para 30bps. Você quer montar um **calendar spread (flattener)** para lucrar com essa compressão.",
